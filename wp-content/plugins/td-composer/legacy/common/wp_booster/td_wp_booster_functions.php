...
if ( $tds_search_taxonomies_terms === 'yes' ) {
    ...
    //
    //    if ( ( !is_admin() && $query->is_main_query() && $query->is_search() ) || $query->get( 'td_block_query' ) === 'search_query' ) {
    //
    //        global $wpdb;
    //
    //        // get all taxonomies
    ...
    //
    //        // get the table name for the term_taxonomy table
    //        $table_name = $wpdb->prefix . 'term_taxonomy';
    //
    //        // get the table name for the term_meta table (if you are using custom fields)
    //        $meta_table_name = $wpdb->prefix . 'termmeta';
    //
    //        // loop through each taxonomy and retrieve the terms that match the search query
    //        $matching_taxes_terms_ids = array();
    //        foreach ( $taxonomies as $taxonomy ) {
    //
    //            // run a custom query to retrieve the term IDs that match the search query
    //            $matching_term_ids = $wpdb->get_col(
    //                $wpdb->prepare("
    //                    SELECT term_taxonomy_id
    //                    FROM $table_name
    //                    INNER JOIN $wpdb->terms ON $table_name.term_id = $wpdb->terms.term_id
    //                    LEFT JOIN $meta_table_name ON $table_name.term_taxonomy_id = $meta_table_name.term_id
    //                    WHERE taxonomy = %s
    //                    AND ( name LIKE '%%%s%%' OR description LIKE '%%%s%%' )
    //                ", $taxonomy, $search_query, $search_query )
    //            );
    //
    //            if ( !empty( $matching_term_ids ) ) {
    //                $matching_taxes_terms_ids = array_merge( array_unique( $matching_term_ids ), $matching_taxes_terms_ids );
    //            }
    //
    //        }
    //
    //        //print_r($matching_taxes_terms_ids);
    //        //die;
    //
    //        // use the matching terms to retrieve the posts that belong to those terms
    //        if ( !empty( $matching_taxes_terms_ids ) ) {
    //
    //            // add the posts join filter
    //            if ( !has_filter( 'posts_join', 'td_search_join' ) ) {
    //                add_filter( 'posts_join', 'td_search_join', 10, 2 );
    //            }
    //
    //            // build additional SQL to search for matching term IDs
    //            $term_ids_sql = implode( ',', array_map( 'intval', $matching_taxes_terms_ids ) );
    //            $search .= " OR ( td_search_tr.term_taxonomy_id IN ( {$term_ids_sql} ) ) ";
    //
    //        }
    //
    //        //print_r($search);
    ...
    //function td_search_join( $join, $wp_query ) {
    //    global $wpdb;
    //    $join .= "LEFT JOIN {$wpdb->term_relationships} AS td_search_tr ON ( {$wpdb->posts}.ID = td_search_tr.object_id )";
    //    return $join;
    //}
